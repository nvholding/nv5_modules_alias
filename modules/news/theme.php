...
function viewcat_grid_new($array_catpage, $catid, $generate_page)
{
    ...
    foreach ($array_catpage as $array_row_i) {
		...
		$danhmuc = $global_array_cat[$array_row_i['catid']]['title'];
		$linkdm = $global_array_cat[$array_row_i['catid']]['link'];
		$xtpl->assign('DANHMUC', $danhmuc);
		$xtpl->assign('LINKDM', $linkdm);
		
		if( !empty( $array_row_i['author'] )){
			$array_row_i['author'] = $array_row_i['author'];
		}else{
			$array_row_i['author'] = 'Vidoco';
		}
