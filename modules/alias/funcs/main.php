...
if (!empty($data['image'])) {
        ...
        $meta_property['og:image'] = NV_MY_DOMAIN . $data['image'];
    }
	
  if (!empty($data['keywords'])) {
        $key_words = $data['keywords'];
    } else {
        $key_words = nv_get_keywords($data['bodytext']);

        if (empty($key_words)) {
            $key_words = nv_unhtmlspecialchars($data['title']);
...
if (! empty($search['key'])) {
    ...
    $page_title = empty($data['titlesite']) ? $data['title'] : $data['titlesite'];
    $description = empty($data['hometext']) ? nv_clean60(strip_tags($data['description']), 160) : $data['hometext'];
